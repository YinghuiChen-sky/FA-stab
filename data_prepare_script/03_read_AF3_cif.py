...
parser.add_argument("--mut_AF3_pred_dir", type=str, default="", help="path of dir predicted struct pdb files")
...
mut_AF3_pred_dir = FLAGS.mut_AF3_pred_dir
...
for mut_sample in mut_sample_list:

    sample_af3 = re.sub(",","",mut_sample.lower())
    cif_path = f"{AF3_pred_dir}/{sample_af3}/{sample_af3}_model.cif"

    # wt_name = sample.rsplit("_",1)[0]
    # pdb_path = f"{mut_AF3_pred_dir}/{wt_name}/{sample}.pdb"
    print(cif_path)

    atom_coords, atom_types, atom_pLDDT, residue_ids, residue_types, seq_tokens, ca_indices = parse_cif_to_atoms(cif_path, chain_id='A')
    print("parse_cif_to_atoms", mut_sample, "atom_coords", atom_coords.shape, "atom_types", atom_types.shape, "atom_pLDDT", atom_pLDDT.shape, "residue_ids", residue_ids.shape, "residue_types", residue_types.shape,
            "seq_tokens", seq_tokens.shape, "ca_indices", ca_indices.shape)

    subgroup = OUT_HDF5.create_group(mut_sample)
    subgroup.create_dataset('target_tokens', data=seq_tokens, dtype=np.int8) #(L,L)
    subgroup.create_dataset('atom_coords', data=atom_coords, dtype=np.float32) #(atoms_num, 3)
    subgroup.create_dataset('atom_types', data=atom_types, dtype=np.int8)  #(atoms_num, )
    subgroup.create_dataset('ca_indices', data=ca_indices, dtype=np.int16)  #(atoms_num, )
    ...
    #print shape
    # print(sample, 'atom_coords', atom_coords.shape, 'atom_types', atom_types.shape, 'atom_pLDDT', atom_pLDDT.shape,
    #         'residue_mapping', residue_ids.shape)
